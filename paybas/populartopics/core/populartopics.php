 ...
 * Class populartopics
 *
 * @package paybas\populartopics\core
 */
	class populartopics
	{
		/**
		...
		*/
		 ...
		 */
		public function display_popular_topics($tpl_loopname = 'popular_topics', $spec_forum_id = 0, $include_subforums = true)
		{

			// can view pt ?
			if ($this->auth->acl_get('u_pt_view') == '0')
			{
				return;
			}

			// if user can enable popular topics and it is not enabled then return
			if ($this->auth->acl_get('u_pt_enable') && isset($this->user->data['user_pt_enable']) && !$this->user->data['user_pt_enable'])
			{
				return;
			}

			//load language
			$this->language->add_lang('populartopics', 'paybas/populartopics');

			// support for phpbb collapsable categories extension
			if ($this->collapsable_categories !== null)
			{
				$fid = 'fid_pt'; // can be any unique string to identify your extension's collapsible element
			...
			$topics_per_page = (int) $this->config['pt_number'];
			if ($this->auth->acl_get('u_pt_number') && isset($this->user->data['user_pt_number']))
			{
				$topics_per_page = (int) $this->user->data['user_pt_number'];
			}

			//number of pages
			$pt_page_numbermax = (int) $this->config['pt_page_numbermax'];
			$nolimitpages = (int) $this->config['pt_page_number'];
			if ($nolimitpages == 0)
			{
				$total_topics_limit = $topics_per_page * $pt_page_numbermax;
			}
			else
			{
				// max 100 pages
			...
			$display_parent_forums = $this->config['pt_parents'];

			//pt block location
			$location = $this->config['pt_location'];
			// if user can set location and it is set then use the preference
			if ($this->auth->acl_get('u_pt_location') && isset($this->user->data['user_pt_location']))
			{
				$location = $this->user->data['user_pt_location'];
			}

			$sort_topics = $this->config['pt_sort_start_time'] ? 'topic_time' : 'topic_last_post_time';
			// if user can set popular topic sorting order and it is set then use the preference
			if ($this->auth->acl_get('u_pt_sort_start_time') && isset($this->user->data['user_pt_sort_start_time']))
			{
				$sort_topics = $this->user->data['user_pt_sort_start_time'] ? 'topic_time' : 'topic_last_post_time';
			}

			$this->unread_only = $this->config['pt_unread_only'];
			if ($this->auth->acl_get('u_pt_unread_only') && isset($this->user->data['user_pt_unread_only']))
			{
				$this->unread_only = $this->user->data['user_pt_unread_only'];
			}

			$ptstart = $this->request->variable($tpl_loopname . '_start', 0);
			...
			$topics_count = $this->gettopiclist(max(0, min((int) $ptstart, $total_topics_limit)) , $topics_per_page, $total_topics_limit, $sort_topics);
			...
			if (sizeof($this->topic_list))
			{
				...
				$sql_array = array(
					...
					'ORDER_BY'  => 't.topic_views DESC, t.' . $sort_topics . ' DESC',
				);
				 ...
				 *
				 * @event paybas.populartopics.sql_pull_topics_data
				 * @var   array    sql_array        The SQL array
				 * @since 2.0.0
				 */
				extract(
					$this->dispatcher->trigger_event(
						'paybas.populartopics.sql_pull_topics_data',
				...
				if (sizeof($rowset))
				{
					 ...
					 *
					 * @event paybas.populartopics.modify_topics_list
					...
					extract(
						$this->dispatcher->trigger_event(
							'paybas.populartopics.modify_topics_list',
					...
					foreach ($rowset as $row)
					{
						 ...
						 *
						 * @event paybas.populartopics.modify_topictitle
						 * @var   array    row      the forum row
						 * @var   string    prefix  the topic title prefix
						 * @since 2.1.3
						 */
						$vars = array('row', 'prefix');
						extract($this->dispatcher->trigger_event('paybas.populartopics.modify_topictitle', compact($vars)));

						//fallback if there is no listener
						if (!$this->is_listening('imkingdavid\prefixed\event\listener', 'paybas.populartopics.modify_topictitle'))
						{
						 ...
						 *
						 * @event paybas.populartopics.modify_tpl_ary
						 * @var   array    row            Array with topic data
						 * @var   array    tpl_ary        Template block array with topic data
						 * @since 2.0.0
						 */
						$vars = array('row', 'tpl_ary');
						extract($this->dispatcher->trigger_event('paybas.populartopics.modify_tpl_ary', compact($vars)));
					...
					$this->pagination->generate_template_pagination($pagination_url, 'pagination', $tpl_loopname . '_start', $topics_count, $topics_per_page, max(0, min((int) $ptstart,$total_topics_limit)));
			...
			$this->template->assign_vars(
				array(
					'PT_SORT_START_TIME'                   => $sort_topics === 'topic_time',
					'S_POPULAR_TOPICS'                      => true,
					'S_LOCATION_TOP'                       => $location == 'PT_TOP',
					'S_LOCATION_BOTTOM'                    => $location == 'PT_BOTTOM',
					'S_LOCATION_SIDE'                      => $location == 'PT_SIDE',
		 ...
		 * @return int
		 */
		private function gettopiclist($ptstart, $topics_per_page, $total_topics_limit, $sort_topics)
		{
			$this->forums = $this->topic_list = array();
			$topics_count = 0;
			$this->obtain_icons = false;
			$excluded_topics = explode(',', $this->config['pt_anti_topics']);
			$min_topic_level = $this->config['pt_min_topic_level'];
			...
			if ($this->unread_only && $this->user->data['user_id'] != ANONYMOUS)
			{
				...
				$ptstart = min(count($unread_topics) - 1 , (int) $ptstart);

				foreach ($unread_topics as $topic_id => $mark_time)
				{
					$topics_count++;
					if (($topics_count > $ptstart) && ($topics_count <= ($ptstart + $topics_per_page)))
					{
			...
			else
			{
				 ...
				 *
				 * @event paybas.populartopics.sql_pull_topics_list
				 * @var   array    sql_array        The SQL array
				 * @since 2.0.4
				 */
				$vars = array('sql_array');
				extract($this->dispatcher->trigger_event('paybas.populartopics.sql_pull_topics_list', compact($vars)));
				...
				if ($result != null)
				{
					$ptstart = min((int) $result->num_rows - 1 , $ptstart);
				}
				else
				{
					$ptstart = 0;
